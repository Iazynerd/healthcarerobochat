...
Future db_conn(name,pass,rem) async {
  ...
  final conn = await MySqlConnection.connect(ConnectionSettings(
      host: ip,  //can be change in the app_const file
      port: 3306,
      user: '', //fill with the right cred.
      db: '', //fill with the right cred.
      password: '')); //fill with the right cred.
  ...
  try {
    ...
    var results = await conn.query(
        'select * from patients where username = ?', [name]);
    if(results.length >= 1){
      for (var row in results) {
        if(pass.toString() == row["password"]) {
          if(rem) {
            ...
            Get.to(() => AudioRecognize(row['id']))?.whenComplete(() => startnoti(row['id']));
        }else{
...
Future search() async {
  ...
  // Query the database using a parameterized query
  var results = await conn.query(
      'select $req from $table where p_id = ?', [id]);
  if (results.length == 1) {
    for (var row in results) {
      result = row[req];
      return result;      
...
class User_data{
  ...
  Future search() async {
    ...
    // Query the database using a parameterized query
    var results = await conn.query(
        'select $req from $table where id = ?', [id]);
