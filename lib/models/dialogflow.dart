...
var redirectUrl = ''; // redirectURl in the case of mobile app is the clientID inverted + :/oauth2redirect
var id;
...
Future<void> getToken(var text) async {
  ...
  print(md);
  var session_id = md + '2' + 'maak' + id.toString() + 'star';
  ...
  final http.Response httpResponse = await http.post(
      Uri.parse(
          'https://global-dialogflow.googleapis.com/v3/projects/sunlit-standard-351419/locations/global/agents/b698773c-05e5-4ff7-96ef-73d0b2a956e5/sessions/$session_id:detectIntent'),
      headers: <String, String>{'Authorization': 'Bearer $token'},
      body: body);
  print(httpResponse.body);
  if (httpResponse.statusCode == 200) {
    var res = jsonDecode(httpResponse.body);
    var the_result = res['queryResult']["responseMessages"][0]['text']['text'][0];
    print(the_result);
      var t = res['queryResult']["responseMessages"];
      if(t.length == 2){
        if(res['queryResult']["responseMessages"][1]['text']['text'][0] == 'كيف تشعر اليوم؟'
        || res['queryResult']["responseMessages"][1]['text']['text'][0] == 'كيف حالك اليوم؟'){
